  ...
  <div class="content-page">
    <div class="content-nav">
      ...
      <div class="operation-nav">
        <el-button type="primary" icon="plus" @click="handleRowPush">添加课程</el-button>
      </div>
    </div>
    ...
    <div class="content-main">
      <div class="filter-box">
        <el-form :inline="true" :model="filterForm" class="demo-form-inline">
          <el-form-item label="课程名称">
            <el-input v-model.trim="filterForm.name" placeholder="课程名称" v-on:input="onSubmitFilter"></el-input>
          </el-form-item>
      ...
      <div class="form-table-box">
        <el-table :data="tableData" style="width: 100%" border stripe>
          ...
          <el-table-column prop="list_pic_url" label="封面" min-width="180">
            <template scope="scope">
              <img v-if="scope.row.list_pic_url" :src="scope.row.list_pic_url" class="image-show" />
            </template>
          </el-table-column>
          <el-table-column prop="retail_price" label="售价¥" min-width="120"></el-table-column>
          <el-table-column prop="is_show" label="上架" min-width="80">
            <template scope="scope">{{ scope.row.is_on_sale == 1 ? '是' : '否'}}</template>
          </el-table-column>
          <el-table-column label="自创服务" min-width="80">
            <template scope="scope">{{ JSON.stringify(scope.row.companyInfo) == '{}' ? '否' : '是'}}</template>
          </el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleRowEdit(index, row) {
      console.log(row);
      this.$router.push({ name: "courseaddpage", query: { id: row.id } });
    },
